...
async def retrieve_node(state: RAGState) -> dict:
    ...
    try:
        ...
        print(f"Retrieval failed (likely unconfigured Pinecone API): {e}")
        return {
            "documents": [],
            "retrieval_scores": [],
        }
...
async def rewrite_query_node(state: RAGState) -> dict:
    ...
    try:
        ...
        print(f"Rewrite query LLM invocation failed (likely missing GROQ_API_KEY): {e}")
        return {
            "rewrite_attempts": rewrite_attempts + 1,
        }
...
async def generate_answer_node(state: RAGState) -> dict:
    ...
    try:
        ...
        print(f"Generate answer LLM invocation failed (likely missing GROQ_API_KEY): {e}")
        answer = "I apologize, but I encountered a configuration error (likely missing GROQ_API_KEY) while attempting to synthesize the answer."
    ...
    for document, score in zip(documents, scores):
        ...
        if source_key not in seen_sources:
            ...
            sources.append({
                ...
                "preview": document.page_content[:280] + "..." if len(document.page_content) > 280 else document.page_content
            })
