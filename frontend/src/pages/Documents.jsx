        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border-subtle pb-5">
          <div className="flex flex-col">
            <h2 className="text-base font-bold text-white">
              {role === 'employee' ? 'Available Documents' : 'Knowledge Ingestion'}
            </h2>
            <p className="text-xs text-text-secondary mt-0.5">
              {role === 'employee' 
                ? 'View indexed documents and files available in your workspace.' 
                : 'Upload PDFs to vectorize and query them instantly.'}
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border-subtle text-xs">
                {filteredDocs.map((doc) => {
                  ...
                  const isSuccess = doc.status === 'Indexed' || doc.status === 'completed';
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-5 font-semibold text-white flex items-center gap-3 min-w-0 max-w-[280px]">
                        ...
                        <span className="truncate group-hover:text-indigo-400 transition-colors">{doc.filename}</span>
                      </td>
                      
                      {/* Size */}
                      <td className="py-3.5 px-4 text-text-secondary font-medium">
                        {formatSize(doc.size || 1024 * 1024)}
                      </td>
